...
function ShareButton({
  ...
  return (
    <div className="relative" ref={dropdownRef}>
      ...
      {open && (
        <div className="absolute left-0 right-0 top-full z-10 mt-2 overflow-hidden rounded-xl border border-vendoroo-border bg-white shadow-lg">
          {platforms.map((platform) => (
            <a
              ...
              className="flex items-center gap-3 px-4 py-3 text-sm text-vendoroo-smoke transition-colors hover:bg-vendoroo-light"
            >
              {platform.icon}
          ...
          <button
            ...
            className="flex w-full items-center gap-3 border-t border-vendoroo-border px-4 py-3 text-sm text-vendoroo-smoke transition-colors hover:bg-vendoroo-light"
          >
            {copied ? (
              <>
              ...
              <>
                <svg className="size-4" fill="none" stroke="currentColor" strokeWidth={2} viewBox="0 0 24 24">
                  <path strokeLinecap="round" strokeLinejoin="round" d="M13.19 8.688a4.5 4.5 0 011.242 7.244l-4.5 4.5a4.5 4.5 0 01-6.364-6.364l1.757-1.757m9.86-2.813a4.5 4.5 0 00-1.242-7.244l-4.5-4.5a4.5 4.5 0 00-6.364 6.364L4.49 8.81" />
                </svg>
                Copy link
              </>
            )}
...
export function ResultsView({ id }: { id: string }) {
  ...
  React.useEffect(() => {
    ...
    if (params.get("share") === "true") {
      const t = setTimeout(() => {
        const btn = document.querySelector("[data-share-button]") as HTMLButtonElement | null;
        btn?.click();
      }, 500);
