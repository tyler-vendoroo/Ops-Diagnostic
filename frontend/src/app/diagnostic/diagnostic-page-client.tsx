...
const ALLOWED_NEXT = ["/diagnostic/quick", "/diagnostic/full"] as const;
...
export function DiagnosticPageClient() {
  ...
  const nextPath =
    rawNext && ALLOWED_NEXT.includes(rawNext as (typeof ALLOWED_NEXT)[number])
      ? rawNext
      : null;

  function handleLeadSubmitted(l: LeadCapture) {
    if (nextPath) {
  ...
  return (
    <div className="mx-auto w-full max-w-4xl flex-1 bg-vendoroo-page px-4 py-12 sm:px-6 sm:py-16">
      ...
      <div className="mt-12">
        {!lead ? (
          <LeadCaptureForm onSubmitted={handleLeadSubmitted} />
        ) : (
